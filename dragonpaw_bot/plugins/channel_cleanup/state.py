...
_cache: dict[int, CleanupGuildState] = {}


def _state_path(guild_id: int) -> Path:
    return STATE_DIR / f"channel_cleanup_{guild_id}.yaml"


def load(guild_id: int) -> CleanupGuildState:
    ...
    if not path.exists():
        st = CleanupGuildState(guild_id=guild_id)
        _cache[guild_id] = st
        return st

    logger.debug("Loading channel cleanup state", guild_id=guild_id, path=str(path))
    ...
    try:
        st = CleanupGuildState.model_validate(data)
    except pydantic.ValidationError:
        logger.exception(
            "Channel cleanup state validation failed", guild_id=guild_id, path=str(path)
        )
        raise
...
def save(guild_state: CleanupGuildState) -> None:
    ...
    try:
        ...
        logger.exception(
            "FAILED to save channel cleanup state",
