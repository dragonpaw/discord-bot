...
- **set** — Register or update your own birthday via a multi-step interactive flow: month (select menu) → day (text input modal) → region (select menu) → timezone (select menu). No year collected. Requires `register_role`. Preserves existing wishlist URL on update.
